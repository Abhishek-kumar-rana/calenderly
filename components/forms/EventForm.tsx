...
export default function EventForm({ event }: { event?: EventType }) {
  ...
  const form = useForm<z.infer<typeof eventFormSchema>>({
    ...
    defaultValues: event
      ? {
          ...event,
          description: event.description ?? '', // Ensure controlled input
        }
      : {
          isActive: true,
          ...
          {event && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  className="cursor-pointer bg-red-500 hover:scale-105 hover:bg-red-700"
                  variant="destructive"
              ...
              <AlertDialogContent className="bg-white text-black rounded-2xl shadow-2xl shadow-accent-foreground">
                <AlertDialogHeader>
